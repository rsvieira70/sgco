...
@section('content')
    <div class="card">
        <div class="card-header">
            <a href="{{ route('teeth.create') }}" class="btn btb-sm btn-success"><i class="fas fa-tooth"></i>
                {{ __('New tooth') }}</a>
        </div>
        <div class="card-body">
            <table id="datatable" class="table table-striped dt-responsive nowrap" style="width:100%">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">{{ __('Tooth code') }}</th>
                        <th class="text-left">{{ __('Tooth name') }}</th>
                ...
                <tbody>
                    @foreach ($teeth as $tooth)
                        <tr>
                            <td class="text-right">{{ $tooth->id }}</td>
                            <td>{{ $tooth->tooth_code }}</td>
                            <td>{{ $tooth->tooth_name }}</td>
                            ...
                            <td class="text-right">
                                @if ($tooth->suspended == null)
                                    <form action="{{ route('teeth.suspend', [$tooth->id]) }}" class="d-inline formSuspend" method="POST">
                                        @method ('PATCH')
                                        @csrf
                                ...
                                @else
                                    <form action="{{ route('teeth.suspend', [$tooth->id]) }}" class="d-inline formReactivate" method="POST">
                                        @method ('PATCH')
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-success"><i class="fas fa-lock-open"></i> {{ __('Reactivate') }}</button>
                                    </form>
                                @endif
                                <a href="{{ route('teeth.edit', [$tooth->id]) }}" class="btn btn-sm btn-info "><i class="fas fa-pencil-alt"></i> {{ __('Edit') }}</a>
                                <form action="{{ route('teeth.destroy', [$tooth->id]) }}" class="d-inline formDelete" method="POST">
                                    @method ('DELETE')
                                    @csrf
