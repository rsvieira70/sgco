...
@section('content')
    <div class="card">
        <div class="card-header">
            <a href="{{ route('departments.create') }}" class="btn btb-sm btn-success"><i class="fas fa-building"></i>
                {{ __('New department') }}</a>
        </div>
        <div class="card-body">
                ...
                <tbody>
                    @foreach ($departments as $department)
                        <tr>
                            ...
                            <td class="text-right">
                                @if ($department->suspended == null)
                                    <form action="{{ route('departments.suspend', [$department->id]) }}" class="d-inline formSuspend" method="POST">
                                        @method ('PATCH')
                                        @csrf
                                ...
                                @else
                                    <form action="{{ route('departments.suspend', [$department->id]) }}" class="d-inline formReactivate" method="POST">
                                        @method ('PATCH')
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-success"><i class="fas fa-lock-open"></i> {{ __('Reactivate') }}</button>
                                    </form>
                                @endif
                                <a href="{{ route('departments.edit', [$department->id]) }}" class="btn btn-sm btn-info "><i class="fas fa-pencil-alt"></i> {{ __('Edit') }}</a>
                                <form action="{{ route('departments.destroy', [$department->id]) }}" class="d-inline formDelete" method="POST">
                                    @method ('DELETE')
                                    @csrf
