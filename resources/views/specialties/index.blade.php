                ...
                <tbody>
                    @foreach ($specialties as $specialty)
                        <tr>
                            ...
                            <td class="text-right">
                                @if ($specialty->suspended == null)
                                    <form action="{{ route('specialties.suspend', [$specialty->id]) }}" class="d-inline formSuspend" method="POST">
                                        @method ('PATCH')
                                        @csrf
                                ...
                                @else
                                    <form action="{{ route('specialties.suspend', [$specialty->id]) }}" class="d-inline formReactivate" method="POST">
                                        @method ('PATCH')
                                        @csrf
                                        <button type="submit" class="btn btn-sm btn-success"><i class="fas fa-lock-open"></i> {{ __('Reactivate') }}</button>
                                    </form>
                                @endif
                                <a href="{{ route('specialties.edit', [$specialty->id]) }}" class="btn btn-sm btn-info "><i class="fas fa-pencil-alt"></i> {{ __('Edit') }}</a>
                                <form action="{{ route('specialties.destroy', [$specialty->id]) }}" class="d-inline formDelete" method="POST">
                                    @method ('DELETE')
                                    @csrf
